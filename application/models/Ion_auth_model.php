...
//echo $hash = $this->hash_password('password', 'miguel.padul@legislatura.gov.ar');
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,re_password')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
...
//			echo "<br>".$this->hash_password($password);die();
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				/* cuando el usuario esta desactivado */
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error('login_unsuccessful_not_active');
							
							redirect('Manager/Usuarios/desactivado', 'refresh');
				...
				/* cuando el usuario no cambio el password */
				...
				$this->session->set_userdata('email_login', $identity);
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select([
			    ...
			    $this->tables['legislaturas'].'.nombre as nombre_legislatura',
			    $this->tables['legislaturas'].'.direccion as direccion_legislatura',
			    $this->tables['legislaturas'].'.telefono as telefono'
			]);
		}
		$this->db->join(
				    $this->tables['legislaturas'],
				    $this->tables['legislaturas'].'.id='.$this->tables['users'].'.id_legislatura',
				    'inner'
				);
...
//var_dump($data);die();
	...
	{
//		$hash = $this->hash_password('password', 'silvana.conte@legislatura.gov.ar');
