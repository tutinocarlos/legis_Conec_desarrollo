...
class Ion_auth
{
	...
	public function forgotten_password($identity)
	{
		...
		$user = $this->where('users.'.$this->ion_auth_model->identity_column, $identity)
					 ->where('active', 1)
					 ->users()->row();
		if ($user)
		{
			...
			if ($code)
			{
				...
				else
				{
					...
					echo 'este es uin posiis';
					
//					la funcion semdMailGmail retorna un estado de envio
					$subject = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject');
					$html = $message;
				$reset = 	semdMailGmail($identity,$subject, $html,$attach=FALSE );
					
					
					var_dump($reset); die;
					/* COMENTADO PARA ACTIVAR EL EMAIL DE GMAIL */
//					if ($this->email->send())
//					{
//						$this->set_message('forgot_password_successful');
//						return TRUE;
//					}
					/* FIN COMENTADO PARA ACTIVAR EL EMAIL DE GMAIL */
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		...
		else
		{
			...
			$legislatura = $this->Legislaturas_model->get_legislatura($user->id_legislatura);
			...
			else
			{
				...
				$asunto = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject');
				
				$create_User = semdMailGmail($email,$asunto, $message,$attach=FALSE );
