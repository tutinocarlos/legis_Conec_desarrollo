...
//        $allowed_mime_type_arr = array('application/pdf','image/gif','image/jpeg','image/pjpeg','image/png','image/x-png');
	...
	public function eliminar_imagen(){
 	...
 	public function import_csv(){
			...
			update representantes origen, representantes destino
			set origen.apellido =destino.nombre, origen.nombre = destino.apellido
			where origen.id_legislatura = 1  and origen.id_representante = destino.id_representante
			*/

//			
//			update representantes origen, representantes destino
//set origen.apellido = destino.nombre, origen.nombre = destino.apellido
//where origen.id_legislatura = 91 and origen.id_representante = destino.id_representante
        
//			$mi_archivo = 'file';
//			$file_ext = pathinfo($_FILES["file"]["name"], PATHINFO_EXTENSION);
//			$file_name = 'id_legis_'.$this->input->post('id_legislatura').'_id_user'.$this->user->id.'_'.date("Y-m-d-H-i-s");
//			$config['upload_path'] = "static/manager/legislaturas/csv";
...
//				$data_add = array_values(array_filter($data_add, "trim"));
				...
				if($this->Legislaturas_model->insertar_representantes('representantes',$data_add)){
					...
					/*guardo el archivo csv */
						...
						if (!$this->upload->do_upload($mi_archivo)) {
										//*** ocurrio un error
										$msg_archivo = $this->upload->display_errors();
//										echo $this->upload->display_errors();
										return;
						}else{
							$msg_archivo = '<br>Se ha creado un copia del archivo';
			//				var_dump($data);
