...
class Tipo_publicacion extends MY_Controller {
			...
			if($this->input->is_ajax_request())
    {
			...
			$seccion 		= $this->load->view('manager/secciones/tipo_publicacion/tipos_publicacion',$datos, TRUE);
			...
			$data = array(
				...
				'header' => $panel['header'],
				'panel' => $panel['panel'],
				'script' => base_url().'static/manager/scripts/tipos_publicacion.js'
				);
		...
		if (!$this->ion_auth->logged_in())
    {
      redirect('auth/login');
			...
			$data = $this->Tipo_publicacion_model->get_edit_tipo($id);
			$user = $this->ion_auth->user()->row();
			...
			$seccion 		= $this->load->view('manager/secciones/tipo_publicacion/edit_tipo',$datos, TRUE);
			...
			$this->form_validation->set_rules('detalle', 'Detalle', 'required|min_length[3]');
				if($this->form_validation->run() === true){
						...
						$datos = array(
				 		...
				 		if($this->Categorias_model->Guardar_datos('tipo_publicacion',$datos )){
                ...
                'estado' => 'error',
							...
							redirect('Manager/Tipo_publicacion', 'refresh');
				}	
			$user = $this->ion_auth->user()->row();
		...
		if($this->input->is_ajax_request())
    {
			...
			if($this->Tipo_publicacion_model->check($this->input->post())){
				$result = array(
					'estado'=>true
				);
	...
	function update_tipo(){
		...
		if($this->input->post("botonSubmit")){
			...
			if($this->form_validation->run() === true){
				...
				if($this->Tipo_publicacion_model->update_tipo_pub('tipo_publicacion',$data )){
